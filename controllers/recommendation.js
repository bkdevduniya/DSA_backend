...
const recommend=async (req,res)=>{
    ...
    if(!token) return res.status(401).json({status:"user logged out"});
    const tokenStatus=await tokens.findOne({token});
    if(!tokenStatus) return res.status(401).json( {status:"invaid user"});
    ...
    if(!finalOutput) return res.status(200).json({status:"no question found"});
    else if(finalOutput=="topic complete") return res.status(200).json({status:"topic complete"});
    const questionDetails=await questions.findOne({title:finalOutput});
    return res.status(200).json({status:"success",questions:[questionDetails]});
    }
    catch(err){
        console.log(err);
